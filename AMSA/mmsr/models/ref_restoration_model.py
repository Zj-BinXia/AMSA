...
class RefRestorationModel(SRModel):
    ...
    def feed_data(self, data):
        ...
        self.img_ref3 = data['img_ref3'].to(self.device)
        self.img_ref4 = data['img_ref4'].to(self.device)
    ...
    def optimize_parameters(self, step):
        ...
        for i in range(len(tp)):
            ...
            self.img_ref_list.append(img_ref_feat)
            self.max_val_list.append(max_val)
        self.output = self.net_g(self.img_in_lq, self.pre_offset_list,
                                 self.img_ref_list, self.max_val_list)
    ...
    def test(self):
        ...
        with torch.no_grad():
            tp = [self.img_ref, self.img_ref1, self.img_ref2, self.img_ref3, self.img_ref4]
            # print("self.match_img_in",self.match_img_in.shape)
            match_img_in = self.net_extractor(self.match_img_in, is_match=True)
            _, _, lh, lw = self.img_in_lq.shape
