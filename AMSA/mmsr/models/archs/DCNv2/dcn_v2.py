...
class DCN_pre_offset(DCNv2):
    ...
    def __init__(self,
                 ...
                 deformable_groups=1,
                 extra_offset_mask=True):
        super(DCN_pre_offset,
              self).__init__(in_channels, out_channels, kernel_size, stride,
                             padding, dilation, deformable_groups)
        self.extra_offset_mask = extra_offset_mask
        channels_ = self.deformable_groups * 1 * self.kernel_size[
            0] * self.kernel_size[1]
        self.conv_offset_mask = nn.Conv2d(
            self.in_channels,
            channels_,
    ...
    def forward(self, x, pre_offset):
        ...
        if self.extra_offset_mask:
            # x = [input, features]
            mask = self.conv_offset_mask(x[1])
            x = x[0]
        else:
            mask = self.conv_offset_mask(x)  # [b,
