...
class RefCUFEDDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            img_in = mod_crop(img_in, scale * 8)
            ...
            tp_w = gt_w
            tp_h = gt_h
            ref_list.append(img_ref)
            for i in range(4):
                tp_w *= 0.8
                tp_h *= 0.8
                tp_ref = tp_ref.resize((int(tp_w), int(tp_h)), Image.BICUBIC)
                tp_ref2 = cv2.cvtColor(np.array(tp_ref), cv2.COLOR_RGB2BGR)
                tp_ref2 = tp_ref2.astype(np.float32) / 255.
                tp_ref2 = mod_crop(tp_ref2, scale)
                tp_ref2 = mmcv.impad(
                    tp_ref2, shape=(gt_h, gt_w), pad_val=0)
                ref_list.append(tp_ref2)
            # data augmentation
            img_in, ref_list = augment(img_in, ref_list, self.opt['use_flip'], self.opt['use_rot'])

        else:
            # for testing phase, zero padding to image pairs for same size
            img_in = mod_crop(img_in, scale * 8)
            img_in_gt = img_in.copy()
            img_ref = mod_crop(img_ref, scale * 8)
            img_in_h, img_in_w, _ = img_in.shape
            img_ref_h, img_ref_w, _ = img_ref.shape
            padding = False
            tp_ref = img_ref.copy()
            tp_ref = tp_ref * 255
            tp_ref = Image.fromarray(
                cv2.cvtColor(tp_ref.astype(np.uint8), cv2.COLOR_BGR2RGB))
            target_h = img_in_h
            target_w = img_in_w
            ...
            for i in range(4):
                ...
                tp_ref2 = mod_crop(tp_ref2, scale)
                tp_ref2 = mmcv.impad(
                    tp_ref2, shape=(target_h, target_w), pad_val=0)
                ref_list.append(tp_ref2)
        ...
        img_in, img_in_lq, img_in_up, img_ref_lq, img_ref_up = totensor(  # noqa: E501
            [img_in, img_in_lq, img_in_up, img_ref_lq, img_ref_up],
            bgr2rgb=True,
            float32=True)
        ref_list = totensor(ref_list,
                            bgr2rgb=True,
                            float32=True)
