...
def augment(img_in, ref_list, hflip=True, rotation=True, flow_list=None):
    ...
    if flow_list is not None:
        img_list = [_augment(img) for img in ref_list]
        ...
        img_list = [_augment(img) for img in ref_list]
        img_in = _augment(img_in)
        if len(img_list) == 1:
            img_list = img_list[0]
        return img_in, img_list
